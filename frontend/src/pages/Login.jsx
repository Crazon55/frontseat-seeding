...
export const Login = () => {
  ...
  const demoLogin = async (email) => {
    setBusy(email);
    try {
      const { data } = await api.post("/auth/dev-session", { email });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#09090b] text-zinc-100 px-6">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#121212] border border-zinc-800/80 rounded-xl p-6 space-y-4">
          <button
            ...
            <>
              ...
              <div className="grid grid-cols-1 gap-2">
                {DEMO_USERS.map((u) => (
                  <button
                    key={u.email}
                    data-testid={`demo-login-${u.label.toLowerCase().replace(/\s+/g, "-")}`}
                    disabled={!!busy}
                    onClick={() => demoLogin(u.email)}
                    className="w-full text-left bg-zinc-900 border border-zinc-800 hover:border-zinc-600 rounded-lg px-3 py-2.5 text-xs transition-colors disabled:opacity-50"
                  >
                    <span className="text-white font-medium">{u.label}</span>
                    <span className="text-zinc-500 ml-2">{busy === u.email ? "Signing in…" : "→"}</span>
                  </button>
                ))}
              </div>
            </>
